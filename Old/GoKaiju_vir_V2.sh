...
if [ "$paired" -eq 1 ]; then
    ...
    do
        ...
        i=$(echo $filename | sed 's/\(.*\)_R[1]_\merged_unique.fastq.gz/\1/')
    echo $filename
    echo ${i}
        echo ""
        echo "### $i Gathering total viral sequences ###"
        echo ""
        echo "kaiju -t node -f DB -a greedy -e 3 -E 0.05 -i ${i}_R1_merged_unique.fastq.gz  -o ${i}_kaiju.txt -v -z $cpu"
        kaiju -t ${NODE} -f ${DB} -a greedy -e 3 -E 0.05 -i $dir/${i}_R1_merged_unique.fastq.gz  -o $out/${i}_kaiju.txt -v -z $cpu
        echo ""
    ...
    # remove by list (unalign)
        ...
        echo ""
        seqkit grep -f $out/${i}_kaiju_classified_list -v $dir/${i}_R1_merged_unique.fastq.gz -o $unclassified/${i}_R1_unclassified.fastq.gz

    # extract by list (align)
        seqkit grep -f $out/${i}_kaiju_classified_list $dir/${i}_R1_merged_unique.fastq.gz -o $classified/${i}_R1_classified.fastq.gz
...
# paired end
    ...
    echo "### $i Gathering total viral sequences ###"
    echo ""
    echo "aiju -t NODE -f DB -a greedy -e 3 -E 0.05 -i ${i}_R1_merged_unique.fastq.gz -j ${i}_R1_merged_unique.fastq.gz -o ${i}_kaiju.txt -v -z $cpu"
    kaiju -t ${NODE} -f ${DB} -a greedy -e 3 -E 0.05 -i $dir/${i}_R1_merged_unique.fastq.gz -j $dir/${i}_R1_merged_unique.fastq.gz -o $out/${i}_kaiju.txt -v -z $cpu
    kaiju-addTaxonNames -t ${NODE} -n ${NAME} -i $out/${i}_kaiju.txt -o  $out/${i}_kaiju_classified.txt -u
    ...
    echo ""
    seqkit grep -f $out/${i}_kaiju_classified_list -v $dir/${i}_R1_merged_unique.fastq.gz -o $unclassified/${i}_R1_unclassified.fastq.gz
    seqkit grep -f $out/${i}_kaiju_classified_list -v $dir/${i}_R2_merged_unique.fastq.gz -o $unclassified/${i}_R2_unclassified.fastq.gz
    # extract by list (align)
    seqkit grep -f $out/${i}_kaiju_classified_list $dir/${i}_R1_merged_unique.fastq.gz -o $classified/${i}_R1_classified.fastq.gz
    seqkit grep -f $out/${i}_kaiju_classified_list $dir/${i}_R2_merged_unique.fastq.gz -o $classified/${i}_R2_classified.fastq.gz
    
    echo "# Create krona file of $i"
    kaiju2krona -t ${NODE} -n ${NAME} -i $out/${i}_kaiju.txt -o $krona/${i%_merged.fastq}_kaiju.krona

    echo "# Create krona plot of $i"
    ktImportText -o $krona/${i}_kaiju.html $krona/${i%_merged.fastq}_kaiju.krona
