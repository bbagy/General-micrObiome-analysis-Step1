...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, trimLeft=c(trimleft.value, trimleft.value),
                     maxN=0, maxEE=c(2,2), truncQ=4, truncLen = 0, minLen = 50, rm.phix=TRUE,compress=TRUE,
                     multithread=T, verbose=TRUE, matchIDs=TRUE) #, truncLen=c(250,250)
head(out)
...
# option 3 for nephele (20210316 작성)
set.seed(100)
errF <- suppressWarnings(learnErrors(filtFs, nreads=1000000, nbases = nbases, randomize = TRUE, multithread=TRUE)) # added option nreads=1000000 from qiime
set.seed(100)
errR <- suppressWarnings(learnErrors(filtRs, nreads=1000000, nbases = nbases, randomize = TRUE, multithread=TRUE)) # added option nreads=1000000 from qiime
