...
# Go_qiime2.sh  --project=[] --fastq=[dir] --clustering=[%]  --pattern=[_001.fastq.gz] --samNum=[50] --db=[db dir]  --paired=[1/2]
# require conda install -c anaconda scikit-learn -n qiime2
# 20210228
# add controlling % cluatering for vsearch
# add export code seqs.fasta
...
if [ "$1" == "-h" ]; then
echo "Usage: `basename $0` --project==[] --fastq=[dir] --clustering=[%] --pattern=[_001.fastq.gz] --fileNum=[50] --db=[db dir]  --paired=[1/2]"
exit 0
fi
...
vsearch=$out/vsearch.${clustering}
...
if [ "$paired" -eq 1 ]; then
    ...
    # fasta
    qiime tools export --input-path $qiime2/6_${project}_aligned_rep-seqs.qza  --output-path $qiime2/10_${project}_exported
...
#====== vsearch after dsds2 =========#
    ...
    --p-perc-identity .${clustering} --p-threads 6 --output-dir vsearch \
    ...
    # fasta
    qiime tools export --input-path $vsearch/4_${project}_vsearch_clustered_aligned_rep-seqs.qza  --output-path $vsearch/8_${project}_vsearch_exported
    ...
    biom convert -i $vsearch/8_${project}_vsearch_exported/feature-table.biom -o $vsearch/8_${project}_vsearch_exported/otutable.${clustering}.txt --to-tsv
    


    sed '1d' $vsearch/8_${project}_vsearch_exported/otutable.${clustering}.txt > $vsearch/8_${project}_vsearch_exported/otutable_cleaned.${clustering}.txt

    ## step 7 qiime2 to phyloseq
    
    Go_qiime2ps.R -p ${project} -b $vsearch/8_${project}_vsearch_exported/otutable_cleaned.${clustering}.txt -x $vsearch/8_${project}_vsearch_exported/taxonomy.tsv -t $vsearch/8_${project}_vsearch_exported/tree.nwk -o $vsearch/8_${project}_vsearch_exported
    conda deactivate
