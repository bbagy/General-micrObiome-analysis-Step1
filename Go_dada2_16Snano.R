...
R1492 <- "CGGTTACCTTGTTACGACTT"
rc <- dada2:::rc


fns <- list.files(path, pattern=".fastq.gz");head(fns)
sample.names <- sapply(strsplit(fns, ".fastq.gz"), `[`, 1)

nops <- file.path(fns, "noprimers", basename(fns))
...
pdf(sprintf("%s/%s.hist.%s.pdf", dada2,project, format(Sys.Date(), "%y%m%d")))
hist(lens, 100)
dev.off()
...
#print(sn)
#plist <- list()
#fn <- list.files(path, pattern=sprintf("%s.fastq.gz", sn), full.names =T)
...
filts <- file.path(filt_path, "noprimers", "filtered", basename(fns))
...
### For R1
derep <- derepFastq(filts, verbose=TRUE) #filtFs

names(derepFs) <- sample.names

print("pass1")
set.seed(100)
err <- suppressWarnings(learnErrors(derep, errorEstimationFunction=PacBioErrfun, BAND_SIZE=32,nreads=1000000, multithread=TRUE)) # added option nreads=1000000 from qiime


print("pass2")

dd <- dada(derep, err=err, BAND_SIZE=32, multithread=TRUE) #pool = TRUE, ‘pooled’
...
tax <- assignTaxonomy(seqtab.nochim, db, taxLevels = c("Kingdom","Phylum", "Class", "Order", "Family", "Genus","Species"), minBoot = 80, verbose = TRUE, multithread = TRUE)
rds <- file.path(sprintf("%s_dada2/%s",project,"2_rds"))
if(!file_test("-d", rds)) dir.create(rds)
...
# count table:
otu <- t(seqtab.nochim);head(otu)
row <- rownames(otu);head(row)
