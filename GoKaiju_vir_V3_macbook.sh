...
# paired end
    ...
    do
        ...
        echo "### $i Gathering total viral sequences ###"
        echo ""
        echo "kaiju -t NODE -f DB -a greedy -e 3 -E 0.05 -i ${i}_R1_merged_unique.fastq.gz -j ${i}_R2_merged_unique.fastq.gz -o ${i}_kaiju.txt -v -z $cpu"
        echo ""
        kaiju -t ${NODE} -f ${DB} -a greedy -e 3 -E 0.05 -i $dir/${i}_R1_merged_unique.fastq.gz -j $dir/${i}_R2_merged_unique.fastq.gz -o ${out}/${i}_kaiju.txt -v -z $cpu
        kaiju-addTaxonNames -t ${NODE} -n ${NAME} -i $out/${i}_kaiju.txt -o  $out/${i}_kaiju_classified.txt -u
        #keep classified list
        grep -v "^@" $out/${i}_kaiju_classified.txt | cut -f2 > $out/${i}_kaiju_classified_list
