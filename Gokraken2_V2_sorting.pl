...
### May 09 2020
# output파일을 paired에서 하나로 출력되는 것을 수정 하고자 한다.
...
foreach my $sample ( keys %s2f )    {
    ...
    my $out1 = "kraken2/$out_dir" . "/" . $sample . "_classifed_out.txt";
    my $log = "kraken2/$out_dir" . "/" . $sample . "_out.log";
    my $list = "kraken2/$out_dir" . "/" . $sample . "_classified_list.txt";
    
    my $out_report = "kraken2/$out_dir_report" . "/" . $sample . "_report.txt";
    my $out_mpa_report = "kraken2/$out_dir_report" . "/" . $sample . "_mpa.txt";
    my $out_class1 = "kraken2/$out_dir_classified" . "/" . $sample . "_R1_classified.fastq.gz";
    my $out_class2 = "kraken2/$out_dir_classified" . "/" . $sample . "_R2_classified.fastq.gz";
    my $out_unclass1 = "kraken2/$out_dir_unclassified" . "/" . $sample . "_R1_unclassified.fastq.gz";
    my $out_unclass2 = "kraken2/$out_dir_unclassified" . "/" . $sample . "_R2_unclassified.fastq.gz";
    ...
    push( @list , $list );
    # run kraken2
    print STDERR "\nRunning:\nkraken2 --db $db --threads $cpu_count  --output $out --use-mpa-style --report $out_mpa_report @fastqs 2> $log\n";
    system( "kraken2 --db $db --threads $cpu_count  --output $out --use-mpa-style --report $out_mpa_report @fastqs 2> $log" );
    
    print STDERR "\nRunning:\n @fastqs \n";
    print "\$fastqs[0] = $fastqs[0]\n";
    print "\$fastqs[1] = $fastqs[1]\n";

    
    
    # extract classified list
    print STDERR "\nExtract classified list:\n grep -v '^@' $out | cut -f2 > $list\n";
    system( "grep  'C' $out > $out1" );
    system( "grep -v '^^@'' $out1 | cut -f2 > $list"); # 원래는 '^@' 인데 '^^@''을 사용   Syntax error: Unterminated quoted string를 해결 하기 위해 변형 되었다.


    # remove by list (unalign)
    print STDERR "\nExtract unclassified sequences(R1):\n seqkit grep -f $list -v $fastqs[0] -o $out_unclass1\n";
    system( "seqkit grep -f $list -v $fastqs[0] -o $out_unclass1");
    print STDERR "\nExtract unclassified sequences(R2):\n seqkit grep -f $list -v $fastqs[1] -o $out_unclass2\n";
    system( "seqkit grep -f $list -v $fastqs[1] -o $out_unclass2");
    
    # extract by list (align)
    print STDERR "\nExtract classified sequences(R1):\n seqkit grep -f $list $fastqs[1] -o $out_class1\n";
    system( "seqkit grep -f $list $fastqs[0] -o $out_class1");
    print STDERR "\nExtract classified sequences(R2):\n seqkit grep -f $list $fastqs[1] -o $out_class2\n";
    system( "seqkit grep -f $list $fastqs[1] -o $out_class2");
