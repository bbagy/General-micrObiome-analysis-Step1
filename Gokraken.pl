#!/usr/bin/perl
# September 27 2018
# original name: run_kraken_macproV4.pl
# 1. --threads 기능이 추가 된다.
# 2. kraken dir에 모든 파일 들이 저장 된다.
# 3. _rel-ab.spf scripts는 수정  한다.
# 4. Aldrovandi'lab server에 사용하도록 metaphlan_to_stamp.pl path를 준다. PATH=${PATH}:/Lab_Share/hpark/help
# 5. make fastq for output file (190123)
# Gokraken.pl -thread 4 -o krakenbac_out -c classified_krakenbac -u unclassified_krakenbac --report krakenbac_mpa.txt -log kraken/krakenbac_log.txt --db /ramdisk/minikraken_20171019_8GB --paired filtered_fastq/*.fastq
...
# add path
use File::Spec;
print File::Spec->join('/Users/heekukpark/microbiome_helper-master'), "\n";
...
system("rm -r kraken");
system( "mkdir -p kraken/$out_dir");
system( "mkdir -p kraken/$out_dir_classified");
system( "mkdir -p kraken/$out_dir_unclassified");
...
foreach my $sample ( keys %s2f )	{
	...
	my $out = "kraken/$out_dir" . "/" . $sample . "_out.txt";
    my $out_class = "kraken/$out_dir_classified" . "/" . $sample . "_classified.fasta";
    my $out_unclass = "kraken/$out_dir_unclassified" . "/" . $sample . "_unclassified.fasta";
	my $log = "kraken/$out_dir" . "/" . $sample . "_out.log";
	...
	print STDERR "\nRunning:\nkraken --db $db $kraken_options --threads $cpu_count --out-fmt paired --fastq-output --classified-out $out_class --unclassified-out $out_unclass --output $out @fastqs 2> $log\n";
	system( "kraken --db $db $kraken_options --threads $cpu_count --out-fmt paired --fastq-output --classified-out $out_class --unclassified-out $out_unclass --output $out @fastqs 2> $log" );
	
}

### now combine all kraken output files into mpa report:
print STDERR "\nRunning:\nkraken-mpa-report --db $db kraken/@out_files > kraken/$report\n";
system( "kraken-mpa-report --header-line --db $db kraken/$out_dir/*_out.txt > kraken/$report" );

my $report_spf = $report; 
$report_spf =~ s/\.txt$/.spf/;

print STDERR "\nRunning:\nmetaphlan_to_stamp.pl kraken/$report > kraken/$report_spf\n";
system( "metaphlan_to_stamp.pl kraken/$report > kraken/$report_spf");

my $stamp = "kraken/$report_spf";
...
my $lc = 0; # line count
open( 'REPORT' , '<' , "kraken/$report_spf") or die "cant open kraken-mpa-report output in spf format $report_spf\n";
while( <REPORT> )    {
