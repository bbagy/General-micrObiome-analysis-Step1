...
foreach my $sample ( keys %s2f )	{
    ...
    system( "kraken2 --db $db --threads $cpu_count --classified-out $out_class --unclassified-out $out_unclass --output $out @fastqs 2> $log" );
    
    system( "kraken-mpa-report --header-line --db ~/DB/kraken_db/minikraken1/ $out > $out_mpa_report" );
