...
if [ "$paired" -eq 1 ]; then
    ...
    echo ${i}
        ...
        echo ""
        echo "kaiju -t node -f DB -a greedy -e 3 -E $evalue -i ${i}$pattern  -o ${i}_kaiju.txt -v -z $cpu"
        kaiju -t ${NODE} -f ${DB} -a greedy -e 3 -E $evalue -i $dir/${i}$pattern   -o ${out}/${i}_kaiju.txt -v -z $cpu
        echo ""
...
#keep classified list # -wE를 하면 정확히 C 만 골라낼수 있다.
    ...
    # remove by list (unalign)
        ...
        echo ""
        seqkit grep -f $out/${i}_kaiju_classified_list -v $dir/${i}$pattern -o $unclassified/${i}_R1_unclassified.fastq.gz

    # extract by list (align)
        seqkit grep -f $out/${i}_kaiju_classified_list $dir/${i}$pattern  -o $classified/${i}_R1_classified.fastq.gz
        echo ""
