#!/bin/bash
# 190822
# bowtie2-build 1_out/Negative.201215.seqs.fna contam/contam
# Goqc_nextseq_fastq [input dir name] [db path] [paired 1/2]
# requrement:
...
do
    ...
    mkdir -p QC/merged_uniq QC/filtered_homopolymer
...
do
    ...
    # homopolymer
    bbduk.sh in1=QC/merged_uniq/${i}_R1_merged_unique.fastq.gz in2=QC/merged_uniq/${i}_R2_merged_unique.fastq.gz  out1=QC/filtered_homopolymer/${i}_R1_homo_filtered_merged_unique.fastq out2=QC/filtered_homopolymer/${i}_R2_homo_filtered_merged_unique.fastq literal=AAAAAA,CCCCCC,GGGGGG,TTTTTT k=6 mm=f

    gzip QC/filtered_homopolymer/${i}_R1_homo_filtered_merged_unique.fastq
    gzip QC/filtered_homopolymer/${i}_R2_homo_filtered_merged_unique.fastq
    done
else
