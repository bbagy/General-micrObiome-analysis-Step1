...
# 20201112
# fixed code for --paired affects for --classified-out and --unclassified-out (for filtering)
# log 파일 out location을 지정해야 하는 불편함을 해결하지 못했다.
...
my $out_raw = "1_out";
my $report = "2_report";
my $out_dir_classified = "3_classified";
my $out_dir_unclassified = "4_unclassified" ;
my $log = "kraken_log";
...
system( "mkdir -p $out_dir/$out_dir_classified");
system( "mkdir -p $out_dir/$out_dir_unclassified");
system( "mkdir -p $out_dir/$report");
system( "mkdir -p $out_dir/$out_raw");
...
foreach my $sample ( keys %s2f )	{
	...
	my $out = "$out_dir/$out_raw" . "/" . $sample . "_out.txt";
    my $log = "$out_dir/$out_raw" . "/" . $sample . "_out.log";

    my $out_report = "$out_dir/$report" . "/" . $sample . "_report.txt";
    my $out_mpa_report = "$out_dir/$report" . "/" . $sample . "_mpa.txt";
    ...
    if($paired == 1){
        ...
        my $out_class = "$out_dir/$out_dir_classified" . "/" . $sample . "_classified#.fastq";
        my $out_unclass = "$out_dir/$out_dir_unclassified" . "/" . $sample . "_unclassified#.fastq";
        
        my $out_class1 = "$out_dir/$out_dir_classified" . "/" . $sample . "_classified_1.fastq";
        my $out_class2 = "$out_dir/$out_dir_classified" . "/" . $sample . "_classified_2.fastq";
        my $out_unclass1 = "$out_dir/$out_dir_unclassified" . "/" . $sample . "_unclassified_1.fastq";
        my $out_unclass2 = "$out_dir/$out_dir_unclassified" . "/" . $sample . "_unclassified_2.fastq";
        
        print STDERR "\nRunning:\nkraken2 --paired --db $db --threads $cpu_count --classified-out $out_class --unclassified-out $out_unclass --report $out_report --output $out @fastqs 2> $log\n";
        system( "kraken2 --paired  --db $db --threads $cpu_count --classified-out $out_class --unclassified-out $out_unclass --output $out --use-mpa-style --report $out_mpa_report @fastqs 2> $log" );
        
        system( "gzip $out_class1" );
        system( "gzip $out_unclass1" );
        system( "gzip $out_class2" );
        system( "gzip $out_unclass2" );
    }else{
        ...
        system( "kraken2 --db $db --threads $cpu_count --classified-out $out_class --unclassified-out $out_unclass --output $out --use-mpa-style --report $out_mpa_report @fastqs 2> $log" );
...
print STDERR "\nRunning:\nmerge_metaphlan_tables.py $out_dir/$report/*_mpa.txt > $out_dir/kraken2_mpa.txt\n";

system( "merge_metaphlan_tables.py $out_dir/$report/*_mpa.txt > $out_dir/kraken2_mpa.txt" );
...
sub fastqs2sample {
	...
	foreach my $s ( keys %s2f )	{
		...
		if ( $num_fastq == 1 )	{
			...
			if ( ( basename($f[0]) =~ m/_R1\.|_R1_|forward|_1.fastq/ ) and ( basename($f[1]) =~ m/_R2\.|_R2_|reverse|_2.fastq/ ) ) {
				### the filenames seem to be PE reads and already ordered as forward and reverse, so continue
			} elsif ( ( basename($f[1]) =~ m/_R1\.|_R1_|forward|_1.fastq/ ) and ( basename($f[0]) =~ m/_R2\.|_R2_|reverse|_2.fastq/ ) ) 	{
